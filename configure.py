...
def main():
    ...
    _LAUNCHPAD_BAZELRC = os.path.join(
        _LAUNCHPAD_WORKSPACE_ROOT, _LAUNCHPAD_BAZELRC_FILENAME
    )
...
def setup_python(environ_cp):
    ...
    ask_python_bin_path = (
        "Please specify the location of python. [Default is " "%s]: "
    ...
    write_action_env_to_bazelrc(
        ...
        tf_lib_prefix + tf_shared_lib_name + os.path.extsep + lib_ext,
    )

    # Write tools/python_bin_path.sh
    with open(os.path.join(_LAUNCHPAD_WORKSPACE_ROOT, "python_bin_path.sh"), "w") as f:
